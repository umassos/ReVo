...
class DMCI(CompressionModel):
    ...
    # def forward(self, x, qp, inject_loss=False, loss_rate=0.0):
        """
            ...
            Args:
                ...
                inject_loss - flag to turn on/off random zero masking
                loss_rate - rate of random zero masking

            Returns:
                Dictionary containing reconstr ucted image, bit rates, and other metrics
            """
        curr_q_enc = self.q_scale_enc[qp:qp + 1, :, :, :]
        curr_q_dec = self.q_scale_dec[qp:qp + 1, :, :, :]
        ...
        # if inject_loss and (loss_rate > 0.0):
        #     mask = (torch.rand_like(y) > loss_rate).float()
        #     y = y * mask
        # #--------------------------------------------------------#
