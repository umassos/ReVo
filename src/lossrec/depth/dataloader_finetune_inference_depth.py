"""
Dataset for depth video packet loss recovery inference.
...
Note on depth file naming:
    Depth videos may carry a `_vis` suffix (e.g. `scene_vis.mp4`).
    The corresponding mask file is expected without the suffix:
    `scene_frame_mask.npy`.
"""
...
class LossRecoveryData(Dataset):
    """PyTorch Dataset for packet-loss recovery inference (Depth).

    Args:
        clean_dir:     Directory of ground-truth depth videos (.mp4 / .avi).
        corrupted_dir: Directory of codec-degraded depth videos (same filenames).
    ...
    def __init__(self, clean_dir, corrupted_dir, mask_dir, num_frames=16, input_size=224):
        ...
        for video_filename in video_files:
            ...
            # Depth videos may have a `_vis` suffix; strip it to find the mask file
            mask_stem = video_name.replace("_vis", "")
            mask_path = os.path.join(mask_dir, mask_stem + "_frame_mask.npy")

            if not os.path.exists(clean_path):
                skipped.append(f"{video_name} (missing clean file)")
                continue
            if not os.path.exists(corrupted_path):
                skipped.append(f"{video_name} (missing corrupted file)")
                continue
            if not os.path.exists(mask_path):
                skipped.append(f"{video_name} (missing mask: {mask_path})")
                continue
